...
# for pipeline comparison. Defaults to the original sample. Mirrors
# test_docling_qwen.py's structure so the two are directly comparable.
...
# Where we will save Groq's JSON output
OUTPUT_DIR = Path(
    "dataset/gov_myscheme/docling_test"
)

OUTPUT_FILE = OUTPUT_DIR / f"{PDF_STEM}_groq.json"

# Groq-hosted model (OpenAI-compatible API). Groq's catalog changes
# frequently -- verified available via client.models.list() at the time
# this was written.
MODEL = "openai/gpt-oss-120b"

# Same cap as test_docling_qwen.py, so the two runs are comparable on
# identical input size.
...
# PROMPT
# ==========================================================
# Identical to test_docling_qwen.py's prompt, so any difference in
# output is attributable to the model, not the prompt.
...
print("\n" + "=" * 80)
print("DOCLING -> GROQ TEST COMPLETE")
print("=" * 80)
...
print("\nNext step:")
print(
    f"Inspect {OUTPUT_FILE.name} and compare "
